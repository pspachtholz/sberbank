...
```{r}
  ...
  test <- train %>% group_by(sub_area) %>% summarize(price_rank_raion = mean(price_rank_raion)) %>% select(sub_area,price_rank_raion) %>% right_join(test, by="sub_area")  
    ...
    # average price rank per raion >= 2014
  train <- train %>% filter(year>=2014) %>% group_by(sub_area) %>% mutate(price_rank_raion_new = rank(mean_price_raion))
  test <- train %>% filter(year>=2014) %>% group_by(sub_area) %>% summarize(price_rank_raion_new = mean(price_rank_raion_new)) %>% select(sub_area,price_rank_raion_new) %>% right_join(test, by="sub_area")  
```
...
exclude_features <- c("price_normal","mean_prices", "count_na_perrow", "train_test", "price_doc", "timestamp", "id", "mean_price_raion_year", "mean_price_raion", "mean_price_persqm_raion", "year", "year_month", "sub_area", "appartment_name", "n", "strange_price","overall_mean")
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  train_set <- train %>% filter(strange_price == 0 & price_normal>0) %>% select(one_of(predictors))
