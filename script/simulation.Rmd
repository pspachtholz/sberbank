...
```{r}
inds <- sample(1:nrow(train_x),nrow(train_x)*0.1)
train_y[inds] <- 6
```
...
plotdf %>% ggplot(aes(x=y))+geom_histogram(aes(y=..density..))+geom_line(stat="density",aes(y=..density..),color="red",size=2)
...
```{r echo=FALSE}
plotdf %>% ggplot(aes(x=x,color=type))+geom_point(aes(y=y), alpha=0.2, size=1)+theme_bw()+coord_cartesian(ylim=c(5,13.5))+geom_point(aes(y=pred), size=2)
```
