...
train <- train %>% group_by(year) %>% mutate(full_norm = full_sq/mean(full_sq))
...
x_train <- subset(train, select = -c(id, timestamp, price_doc, price_persqm, price_doc2, full_sq2, appartment_price, year, appartment_name, strange_build_year, n, build_year, full_sq))
x_test <- subset(test, select = -c(id, timestamp, year, appartment_name, strange_build_year, n, life_sq2, build_year, full_sq))
