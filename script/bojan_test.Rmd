...
m1=floor(mean(re_investment$n1M[re_investment$ts>='2015-01'])/8*nrow(train)) #undersampling by magic numbers
m2=floor(mean(re_investment$n2M[re_investment$ts>='2015-01'])/2*nrow(train)) #undersampling by magic numbers
...
x_train <- subset(train, select = -c(id, timestamp, price_doc, price_persqm, price_doc2, full_sq2, appartment_price, appartment_name, strange_build_year, n, year, year_month))
x_test <- subset(test, select = -c(id, timestamp, appartment_name, strange_build_year, n, year, year_month))
...
train2 %>% ggplot(aes(x=as.numeric(year_quarter), y=price_doc, color=product_type))+geom_point()
```

```{r}
x_test2 <- data.frame(full_norm=seq(range(train$full_norm)[1],range(train$full_norm)[2],by=1))
dtest = xgb.DMatrix(as.matrix(x_test2))
prediction <- exp(predict(gbdt,dtest))-1
plotdf<-data.frame(full=x_test2$full_norm, pred=prediction)
...
cor(tmp2$m1,tmp2$m2)
ggplot(tmp2, aes(x=m1,y=m2))+geom_point()+geom_smooth(method="lm")
```
