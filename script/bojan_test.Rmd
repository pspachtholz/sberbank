...
x_train <- select(train, -c(id, timestamp, price_doc, price_persqm, price_doc2, full_sq2, appartment_price, appartment_name, strange_build_year, n, year, year_quarter, build_year)) 
x_test <- subset(test, select = -c(id, timestamp, appartment_name, strange_build_year, n, year, year_quarter, life_sq2, build_year))
...
train_test <- train_test %>% group_by(product_type, year_month) %>% mutate_all(funs(normalized=(.)-mean(.,na.rm=T))) %>% select(product_type, year_month, contains('normalized')) %>% ungroup()
...
### debugging
basic_features <- str_c(c("full_sq", "life_sq", "num_room"),"_normalized")
x_train <- x_train %>% select(product_type, one_of(basic_features))
x_test <- x_test %>% select(product_type, one_of(basic_features))
...
train2 %>% ggplot(aes(x=log(price_doc+1),y=pred))+geom_point()+coord_cartesian(xlim=c(12,18.5),ylim=c(14,18.5))+geom_abline(slope=1,intercept=0, color="darkred")+facet_grid(product_type~year)

train2 %>% ggplot(aes(x=full_sq, y=abs(pred-log(price_doc+1)))) + geom_point(stat="summary", size=2)+stat_smooth()+facet_grid(product_type~.)

train2 %>% ggplot(aes(x=life_sq, y=abs(pred-log(price_doc+1)))) + geom_point(stat="summary", size=2)+stat_smooth()+facet_grid(product_type~.)

train2 %>% ggplot(aes(x=floor, y=abs(pred-log(price_doc+1)))) + geom_point(stat="summary", size=2)+stat_smooth()+facet_grid(product_type~.)

train2 %>% ggplot(aes(x=num_room, y=abs(pred-log(price_doc+1)))) + geom_point(stat="summary", size=2)+stat_smooth()+facet_grid(product_type~.)

```


```{r, fig.width=10, fig.height=7}
train2 %>% ggplot(aes(x=exp(pred)-1))+stat_density(geom="line", color="darkgreen")+facet_grid(product_type~year)+stat_density(aes(x=price_doc),geom="line", color="darkred")+xlim(c(0,1.5e+07))
```
