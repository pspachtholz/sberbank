---
title: "Simple Data Exploration [R]"
...
* [Association of Variables with Sale Price](#association)
  * [Timestamp](#timestamp)
  * [Interactive Time Plot](#timeplot)
  ...
  * [Floor](#floor)
  * [Max Floor](#maxfloor)
  * [Floor/Max Floor](#floorbymaxfloor)
  * [Appartment State](#state)
  * [Wall Material](#material)
  * [Product Type](#type)

There are a number of outliers in some variables.
Therefore I first plotted the complete data, to get a first impression and then I zoomed into the relevant part of the data distributions. 
...
#### Per Day
Hard to see anything in this plot.
...
### Per day {.tabset #timeplot}
#### Moving Average Plot
It is hard to get anything from the 'per day' plot above, because the resolution of a static plot
is not good enough. So here is a interactive plot using [dygraphs](http://rstudio.github.io/dygraphs/).

Still the time series plot is quite spiky so I decided to apply a moving average filter.
...
### Total Area {.tabset #totalarea}

#### Complete (with Outliers)
It is obvious that there is an outlier here regarding the total area. I would definitely remove/recode this value.
...
Zooming in shows a clear relation, as expected, between the total area and the sale price: 
...
### Living Area {.tabset #livingarea}

#### Complete (with Outliers)

It is obvious that there is an outlier here regarding the living area. I would definitely remove/recode this value.
...
### Kitchen Area {.tabset #kitchenarea}

#### Complete (with Outliers)

Again there are some outliers in kitchen area: Some of them look like years (1970, 1974, 2013, 2014) :-).
...
### Number of Living Rooms {.tabset #livingrooms}

The higher the number of living rooms, the higher sale prices are. 
...
### Floor by MaxFloor {.tabset #floorbymaxfloor}
...
```{r warning = FALSE, echo = FALSE, message = FALSE, cache = FALSE}
train <- train %>% mutate(build_year=as.numeric(build_year),build_year=ifelse(build_year < 1960 | build_year > 2018,NA,build_year))
train %>% filter(product_type=="Investment") %>% ggplot(aes(x=build_year,y=price_doc,color=as.factor(state),fill=as.factor(state)))+facet_grid(as.factor(product_type)~.)+geom_smooth()+theme_bw()

```


### Wall Material {.tabset #material}
...
```{r warning = FALSE, echo=FALSE, fig.width=4, fig.height=4,message=FALSE, cache=FALSE}
