...
train$full_sq[train$id %in% c(8799,10489)] <- train$full_sq[train$id %in% c(8799,10489)]*10

#arrange(train,desc(price_persqm)) %>% select(price_persqm, price_doc, full_sq, product_type, year, everything()) %>% View()
...
#arrange(train,price_persqm) %>% select(price_persqm, price_doc, full_sq, product_type, year, everything()) %>% View()

#arrange(train,desc(product_type),price_persqm) %>% select(price_persqm, price_doc, full_sq, product_type, year, everything()) %>% View()
...
x_train <- subset(train, select = -c(id, timestamp, price_doc, price_persqm))
